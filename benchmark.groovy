...
////secondQ = g.V.and(_().has("ELECTRONS.mixing_beta", T.gt, 0.0f), _().has("CONTROL.max_seconds", T.gt, 0.0f))
//secondQ = g.V.has("ELECTRONS.mixing_beta", T.gt, 0.0f).has("CONTROL.max_seconds", T.gt, 0.0f)
//
...
startSecond = new Date().time
//fifthQ = g.V.has("element", "F").out.out.has('energy', T.lte, 0.0f).as('parent').in.in.as('child2').in.as('child3').in.as('child4').in.as('child5').select.gather{it.size()}
